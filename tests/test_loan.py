...
class TestLoan(unittest.TestCase):
    ...
    def test_single_loan_no_coll_updates(self):
        ...
        loan.init_loans()
        self.assertEqual(len(loan.Loan.active_loans[0].coll_updates_history), 1, "Should be 1")

    def test_single_loan_single_coll_updates(self):
        loan.set_test_mode('loans_2.csv')
        loan.init_loans()
        self.assertEqual(len(loan.Loan.active_loans[0].coll_updates_history), 3, "Should be 3")
    ...
    def test_multiple_loans_multiple_coll_updates(self):
        ...
        self.assertEqual(len(loan.Loan.active_loans[0].coll_updates_history), 2, "Should be 2")
        self.assertEqual(len(loan.Loan.active_loans[1].coll_updates_history), 4, "Should be 4")
