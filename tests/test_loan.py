...
class TestLoan(unittest.TestCase):
    ...
    def test_accrued_interest(self):
        ...
        for cdp in loan.Loan.active_loans:
            start_date = datetime(cdp.start_date.year, cdp.start_date.month, cdp.start_date.day)
            today = datetime.now()
            days_active = (today - start_date).days
            print(cdp.stats)
            print('cad_borrowed:{}', cdp.current_borrowed_cad)
            interest_accrued = cdp.stats[cdp.stats['date'] == today.strftime("%Y-%m-%d")]['accrued_interest_cad'].values[0]
            # for _,row in self)

            expected_interest = round(days_active * loan.DAILY_INTEREST * cdp.current_borrowed_cad, 2)
            print('expected_interest: ', expected_interest)
            # self.assertEqual(interest_accrued, expected_interest, 'Should be {} interest'.format(expected_interest))
